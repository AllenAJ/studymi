...
export const QuizView: React.FC<QuizViewProps> = ({ questions }) => {
  ...
  if (completed) {
    return (
      <div className="flex flex-col items-center justify-center p-16 bg-white dark:bg-darkCard rounded-none shadow-soft border border-softBorder dark:border-darkBorder max-w-2xl mx-auto text-center animate-scale-in">
        <div className="w-40 h-40 bg-iceGray dark:bg-primaryGold/20 rounded-none flex items-center justify-center mb-10 shadow-inner">
        ...
        <button 
          onClick={resetQuiz}
          className="flex items-center gap-3 px-10 py-5 bg-deepNavy dark:bg-white text-white dark:text-deepNavy rounded-none font-bold text-lg hover:scale-105 transition-all shadow-xl"
        >
  ...
  return (
    <div className="w-full max-w-4xl mx-auto animate-slide-up">
      <div className="mb-8 flex justify-between items-center px-4">
        <span className="text-xs font-bold tracking-widest uppercase text-steelGray dark:text-darkMuted bg-white dark:bg-darkCard px-3 py-1 rounded-none border border-softBorder dark:border-darkBorder">
      ...
      <div className="bg-white dark:bg-darkCard rounded-none shadow-soft border border-softBorder dark:border-darkBorder overflow-hidden p-8 md:p-14">
        ...
        <div className="space-y-4">
          {currentQuestion.options.map((option, index) => {
            ...
            let icon = <div className="w-6 h-6 rounded-none border-2 border-softBorder dark:border-darkBorder" />;

            if (showResult) {
              if (index === currentQuestion.correctAnswerIndex) {
                stateClass = "border-green-500 bg-green-50 dark:bg-green-900/20 text-green-900 dark:text-green-400";
                icon = <div className="w-6 h-6 rounded-none bg-green-500 flex items-center justify-center text-white"><Check className="w-4 h-4" /></div>;
              } else if (index === selectedOption) {
                stateClass = "border-red-300 bg-red-50 dark:bg-red-900/20 text-red-900 dark:text-red-400";
                icon = <div className="w-6 h-6 rounded-none bg-red-500 flex items-center justify-center text-white"><X className="w-4 h-4" /></div>;
              } else {
            ...
            return (
              <button
                ...
                disabled={showResult}
                className={`w-full text-left p-6 rounded-none border-2 transition-all flex justify-between items-center group dark:text-white ${stateClass}`}
              >
        ...
        {showResult && (
          <div className="mt-10 flex justify-end animate-fade-in">
            <button
              ...
              className="px-10 py-4 bg-primaryGold text-white rounded-none font-bold hover:bg-yellow-600 transition-all shadow-lg shadow-yellow-200/50 flex items-center gap-2 hover:translate-x-1"
            >
